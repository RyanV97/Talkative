...
class StringSelectionList(parentScreen: TalkativeScreen, x: Int, y: Int, width: Int, height: Int, private var onSelectionChange: ((selection: StringEntry) -> Unit?)? = null) : WidgetList<TalkativeScreen>(parentScreen, x, y, width, height) {
    ...
    class StringEntry(val parentList: StringSelectionList, val value: String) : AbstractWidget(0, 0, parentList.width, 15, Component.empty()) {
        override fun renderButton(poseStack: PoseStack?, mouseX: Int, mouseY: Int, partialTicks: Float) {
            ...
            GuiComponent.drawString(poseStack, Minecraft.getInstance().font, value, x + 2, y + 3, if (parentList.selectedEntry == this) 0x33d4ff else if (isMouseOver(mouseX.toDouble(), mouseY.toDouble())) 0xFFFFFF else 0xCCCCCC)
        }
