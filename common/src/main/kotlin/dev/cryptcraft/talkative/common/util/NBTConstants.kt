package dev.cryptcraft.talkative.common.util

object NBTConstants {
    const val ACTOR_DATA = "TalkativeActorData"
    const val ACTOR_DATA_VERSION = "ActorDataVersion"

    const val DISPLAY_DATA = "display_data"
    const val DISPLAY_NAME_OVERRIDE = "name_override"
    const val DISPLAY_NAME = "actor_name"
    const val DISPLAY_CONVERSATION = "display_in_conversations"
    const val DISPLAY_ENTITY = "display_as_entity"
    const val DISPLAY_TEXTURE = "display_texture"
    const val DISPLAY_TEXTURE_SIZE = "display_texture_size"
    const val MARKER_DATA = "marker_data"
    const val MARKER_LOCATION = "marker_location"
    const val MARKER_COLOUR = "marker_overlay_colour"
    const val MARKER_OFFSET = "marker_offset"

    const val BRANCH_REFERENCES = "branch_references"
    const val BRANCH_NODES = "branch_nodes"
    const val BRANCH_FILE = "branch_file"
    const val BRANCH_INDEX = "branch_index"
    const val BRANCH_HIGH_ID = "branch_highest_id"

    const val NODE_ID = "node_id"
    const val NODE_TYPE = "node_type"
    const val NODE_CHILDREN = "node_children"
    const val NODE_CHILD_TYPE = "node_child_type"
    const val NODE_COMMANDS = "node_commands"
    const val NODE_TEXT_CONTENTS = "node_text_contents"
    const val NODE_BRIDGE_DESTINATION_ID = "node_bridge_destination_id"
    const val NODE_BRIDGE_DESTINATION_BRANCH = "node_bridge_destination_branch"

    const val CONDITIONAL = "conditional"
    const val CONDITIONAL_EXPRESSIONS = "expressions"
    const val CONDITIONAL_HOLDER_BRANCH = "branch_path"
    const val CONDITIONAL_HOLDER_ID = "id"
    const val EXPRESSION_PROP = "property"
    const val EXPRESSION_VALUE = "value"
    const val EXPRESSION_OPERATION = "operation"
}